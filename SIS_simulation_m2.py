...
import matplotlib.pyplot as plt  # Used in plot_function
import os  # Used in openFile
import sys  # Used in openFile
from math import sqrt  # Used in calc_steady_state
...
class Graph:
    ...
    def run_simulation(self, p1, p2):
        ...
        self.sd = (
            stdev(self.sim_tot[self.no_of_steps // 2 :]) if self.sim_tot[-1] > 0 else 0
        )
        return
...
class SubPopulation:
    ...
    def add_edge(self, node1, node2, directed):  # Add edge node1 -> node2
        ...
        if self.directed is False:  # also add "mirror" edge
    ...
    def init_simulation(self):
        ...
        hundred_procent_infected = False
        if hundred_procent_infected:  # if True, start with 100% infected
    ...
    def simulation_step(self, other, step_no):
        ...
        for j in range(self.num_of_nodes):
            ...
            if self.node[j] == 0:  # Susceptible node, can be infected
                ...
                assert (
                    self.beta * contacts <= 1
                ), f"β.(k.v) > 1, with {self.beta}*{contacts}"
