...
beta1, delta1 = sis.calc_beta_delta(r0_1, k)
beta2, delta2 = sis.calc_beta_delta(r0_2, k)
...
g.calc_steady_state(p1, p2)   # calculates Diversity index D, v(∞)
...
for j in range(no_of_runs):
    ...
    print(f"Run no. {j:0=2d}, v(∞): μ = {g.mu:.5f}, σ = {g.sd:.5f}")
...
print()
print("R₀; D; R₁; R₂; v(∞); μ(v); σ(v); #steps; #nodes; #runs; network=directed")
print(f"{r0}; {D}; {r0_1}; {r0_2}; {g.v_8:.5f}; {mu:.5f}; {sd:.5f}; {no_of_steps}; {network_size}; {no_of_runs}; {directed}")
